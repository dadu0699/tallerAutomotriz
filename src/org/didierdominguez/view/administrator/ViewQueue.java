...
public class ViewQueue extends Stage {
    private static ViewQueue instance;
    ...
    public static ViewQueue getInstance() {
        if (instance == null) {
    ...
    private GridPane getViewPriorityQueue() {
        ...
        columnCar.setPrefWidth(x/10);
        columnCar.setCellValueFactory(new PropertyValueFactory<>("car"));
        TableColumn<Order, Customer> columnCustomer = new TableColumn<>("CLIENTE");
        columnCustomer.setPrefWidth(x/10);
        ...
        tableViewPriorityQueue.getColumns().addAll(columnID, columnCar, columnCustomer);
    ...
    private GridPane getViewService() {
        ...
        JFXButton buttonDelete = new JFXButton("FINALIZAR SERVICIO");
        buttonDelete.getStyleClass().addAll("customButton", "primaryButton");
        buttonDelete.setButtonType(JFXButton.ButtonType.FLAT);
        buttonDelete.setPrefSize(x, y);
        buttonDelete.setOnAction(event -> {
            ControllerAttentionQueue.getInstance().removeQueue();
            updateTableViewItemsPriorityQueue();
            updateTableViewItemsServiceQueue();
            updateTableViewServiceFinished();
        });

        hBoxButtons.getChildren().addAll(buttonDelete);
        ...
        columnID.setCellValueFactory(new PropertyValueFactory<>("id"));
        TableColumn<Order, Car> columnCar = new TableColumn<>("CARRO");
        columnCar.setPrefWidth(x/7);
        columnCar.setCellValueFactory(new PropertyValueFactory<>("car"));
        TableColumn<Order, Customer> columnCustomer = new TableColumn<>("CLIENTE");
        columnCustomer.setPrefWidth(x/7);
        columnCustomer.setCellValueFactory(new PropertyValueFactory<>("customer"));
        TableColumn<Order, Employee> columnEmployee = new TableColumn<>("MECÁNICO");
        columnEmployee.setPrefWidth(x/10);
        ...
        columnCarF.setPrefWidth(x/10);
        columnCarF.setCellValueFactory(new PropertyValueFactory<>("car"));
        TableColumn<Order, Customer> columnCustomerF = new TableColumn<>("CLIENTE");
        columnCustomerF.setPrefWidth(x/10);
        columnCustomerF.setCellValueFactory(new PropertyValueFactory<>("customer"));
        TableColumn<Order, Employee> columnEmployeeF = new TableColumn<>("MECÁNICO");
        columnEmployeeF.setPrefWidth(x/10);
        ...
        columnService.setPrefWidth(x/10);
        ...
        tableViewServiceFinished.getColumns().addAll(columnIDF, columnCarF, columnCustomerF, columnEmployeeF, columnService);
        tableViewServiceFinished.setPrefSize(x, 7 * y / 8);
        gridPane.add(tableViewServiceFinished, 0, 4);
